...
function createEventsItemViewTemplate(point, offers, destination) {
  const { basePrice, dateFrom, dateTo, isFavorite, type } = point;
  const { name } = destination;

  return `<li class="trip-events__item">
       <div class="event">
       <time class="event__date" datetime=${dateFrom}>${humanizeTaskDueDate(dateFrom, DATE_FORMAT)}</time>
         ...
         <div class="event__schedule">
           <p class="event__time">
             <time class="event__start-time" datetime=${dateFrom}>${humanizeTaskDueDate(dateFrom, TIME_FORMAT)}</time>
              &mdash;
